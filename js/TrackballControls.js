 ...
 * This was modified to become an ES6 module and fit worldmap proper needs  */
class TrackballControls {
  ...
  getMouseProjectionOnBall(clientX, clientY) {
    ...
    // Build projection
    const projection = this.object.up.clone().setLength(mouseOnBall.y);
  ...
  rotateCamera() {
    ...
    if (angle) {
      const axis = (new THREE.Vector3()).crossVectors(this._rotateStart, this._rotateEnd).normalize();
  ...
  panCamera() {
    ...
    if (mouseChange.lengthSq()) {
      ...
      const pan = this._eye.clone().cross(this.object.up).setLength(mouseChange.x);
  ...
  mousedown(event) {
    ...
    // Transform according to current state
  ...
  mouseup(event) {
    ...
    event.stopPropagation();
    this._state = this.STATE.NONE;
    // Restore hasMoved flag to free country click
